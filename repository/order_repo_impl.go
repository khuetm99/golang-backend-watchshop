...
func NewOrderRepo(sql *db.SQL) OrderRepository {
...
func (o *OrderRepoImpl) UpdateOrder(context context.Context, order model.Order) error {
	// status, order_id, user_id
	sqlStatement := `
		UPDATE orders
		SET 
...
func (o *OrderRepoImpl) ListOrderByUserId(context context.Context, userId string) ([]model.Order, error) {
	sqlOrders := `
		SELECT
		  	orders.user_id,
			 orders.order_id,
...
func (o *OrderRepoImpl) RemoveCard(context context.Context, userId string, productId string, orderId string)  error {
	sqlStatement := `
			DELETE FROM card
		WHERE 
			order_id IN (
	...
	count, _ := result.RowsAffected()
	if count == 0 {
		return errors.New("Remove thất bại")
	}
